...
/**
 * Partner — new factory-intake / stock-count receipt (backlog 4.23, moved under the
 * "الاستلام من المصنع"/"الجرد" tabs by backlog 5.4). AGENT only. Two ways to build the line
 * list before one shared preview + confirm step:
 *   1. "إدخال يدوي" — search the catalog (same `/api/partner/inventory` endpoint the stock
 ...
 *
 * `?kind=COUNT` (linked from the "الجرد" tab's "جرد جديد" button) preselects the count radio.
 */
...
export default function PartnerNewReceiptPage() {
  return (
    <React.Suspense fallback={<Skeleton className="h-64 w-full rounded-2xl" />}>
      <PartnerNewReceiptPageInner />
    </React.Suspense>
  );
}

function PartnerNewReceiptPageInner() {
  ...
  const [kind, setKind] = React.useState<"FACTORY" | "COUNT">(
    searchParams.get("kind") === "COUNT" ? "COUNT" : "FACTORY"
  );
  ...
  async function submitReceipt() {
    ...
    try {
      ...
      router.push(`/partner/stock/intake/${json.data.id}`);
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-lg font-extrabold text-ink">استلام جديد</h1>
      <p className="-mt-4 text-sm text-ink-soft">سجّل استلاماً من المصنع أو جرداً فعلياً — راجع المعاينة قبل التأكيد.</p>
          ...
          <TabsContent value="upload">
            ...
            >
              ...
              <p className="text-xs text-ink-soft">
                استخدم ملف &quot;تصدير المخزون&quot; من صفحة المخزون كقالب — عمود{" "}
