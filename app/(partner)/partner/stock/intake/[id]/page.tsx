...
/**
 * Partner — receipt detail (backlog 4.23, moved under `/partner/stock/intake/[id]` by
 * backlog 5.4). The printable record of one applied `StockReceipt`: header (kind, reference,
 * notes, date), the line list with the previous → new stock per variant, and totals. AGENT
 * only, ownership-checked server-side by `GET /api/partner/receipts/[id]`. Printing relies on
 * `PartnerShell`'s own `data-partner-chrome` + `print:hidden` chrome (no page-local print
 * stylesheet needed).
 */
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
        <div>
          <h1 className="text-lg font-extrabold text-ink">إيصال المخزون</h1>
          <p className="text-sm text-ink-soft">السجل الدائم لهذا الاستلام أو الجرد — قابل للطباعة.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button type="button" variant="outline" className="rounded-xl" onClick={() => router.push("/partner/stock/intake")}>
        ...
        >
          <div className="space-y-6 p-4 sm:p-[22px]">
            <dl className="grid grid-cols-2 gap-4 rounded-xl border border-stone-200 bg-stone-50 p-4 sm:grid-cols-4">
              ...
              {receipt.totalCostPiastres != null && (
                <div>
                  <dt className="text-xs text-ink-soft">إجمالي التكلفة</dt>
                  <dd className="font-bold text-ink" dir="ltr">
                    {formatNumberEn(piastresToEgp(receipt.totalCostPiastres))} ج.م
                  </dd>
                </div>
              )}
              {receipt.notes && (
