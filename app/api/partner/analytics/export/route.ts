...
export async function GET(req: NextRequest) {
  try {
    ...
    if (report === "summary") {
      ...
      return csvResponse(csv, `partner-analytics-summary-${stamp}.csv`);
    }

    if (report === "revenue") {
      const rows = await queries.getRevenueOverTime(granularity, from, to, scope);
      const lines = [
      ...
      return csvResponse(lines.join("\n"), `partner-analytics-revenue-${stamp}.csv`);
    }

    if (report === "products") {
      const rows = await queries.getProductVariantReport(from, to, scope);
      const lines = [
