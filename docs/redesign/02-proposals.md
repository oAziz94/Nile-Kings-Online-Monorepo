...
| Order tracking timeline on customer profile — widened scope | Approved (widened 2026-09-10) | Originally just a visual status trail on the existing list. Widened per Phase 0 review: `profile/orders.md` found no order-detail page at all (clicking a row does nothing), no pagination (unbounded history loads every time), no reorder button, and no price-breakdown view (only the final total, not subtotal/discount/shipping/COD fee even though the API already returns them). Now bundles: a real order-detail page, pagination, and a reorder button, alongside the original tracking-timeline visual |
| PDP image gallery (multi-image per variant/color) | Approved (2026-09-10) | `products-pdp.md`: PDP shows exactly one static image per color today, no gallery, no zoom — flagged as a real gap vs. apparel-ecommerce competitor norms. Requires admin-side work too (multi-image upload per variant; today it's one image per color) |
| Back-in-stock notifications | Approved (2026-09-10) | Out-of-stock variants today are a dead end (disabled chip only). Capture phone/email against a specific out-of-stock variant, notify when restocked — SMS/WhatsApp likely fits this market better than email. New small backend piece: subscriptions table + a trigger on restock |
| International account phone numbers (login/register/password-reset) | Approved (2026-09-10) | Today only Egyptian mobile numbers are accepted anywhere, including account identity — but Gulf-based Egyptian expats buying for family in Egypt is a real segment. Account phone now accepts all 22 countries already listed in the (currently decorative) dropdown, via `libphonenumber-js` per-country validation. The separate delivery-address phone (courier-facing) stays Egypt-only. ~~**Needs a pre-launch action from the user**: enable Twilio Geographic Permissions for the target countries, since Twilio blocks international SMS by default.~~ **Superseded 2026-09-10 — see the "WhatsApp OTP via WaPilot" swap below**: OTP now goes over WhatsApp, not SMS, which doesn't carry Twilio's per-country blocking behavior. See `04-decisions.md` 2026-09-10 |
| COD fee shown as its own checkout line item (not folded into shipping) | **Rejected** (2026-09-10) | User explicitly kept current behavior — COD surcharge stays folded into the "رسوم الشحن" display line. See `checkout.md` Notes for the underlying mechanism (must still replicate the fold-in exactly, not model it as a real shipping-rate difference) |
...
| Twilio SMS → WaPilot WhatsApp for OTP delivery | Approved (2026-09-10) | User decision: drop Twilio entirely, verify account phones over WhatsApp instead. WaPilot already exists as a vendor relationship in this codebase (removed 2026-09-09 from the unrelated partner-order-notification feature — same API reused here: `POST {apiUrl}/{instanceId}/send-message`, `chat_id` = E.164 phone minus `+` plus `@c.us`). Also reinstates OTP verification at registration (dropped as dead code in backlog 4.2, now rebuilt against WhatsApp instead of the old Egypt-only Twilio path). See `04-decisions.md` 2026-09-10 "WhatsApp OTP via WaPilot" for the full scope |
