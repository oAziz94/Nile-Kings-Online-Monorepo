...
| Order/RoutedOrder status propagation at contradictory edges | Proposed, PM recommendation pending user confirmation | See `04-decisions.md` 2026-09-10 for the full reasoning. Recommendation: don't merge the two models (routing pipeline state is legitimately orthogonal to commercial state mid-flight), but auto-propagate at the two edges where divergence is nonsensical — cancelling the order auto-cancels its routing record; the routing record reaching DELIVERED auto-marks the order DELIVERED. Everywhere else, just surface both statuses side by side wherever either appears |
