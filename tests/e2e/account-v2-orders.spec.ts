...
test("cancel on CREATED succeeds from the UI and releases the reservation", async ({ page }) => {
  await loginViaUi(page);
  await page.goto("/profile/orders");

  const card = page.locator("article", { hasText: `#${orderCreatedUiId.slice(-8).toUpperCase()}` });
  await card.getByRole("button", { name: "إلغاء الطلب" }).click();

  const dialog = page.getByRole("dialog", { name: `إلغاء الطلب #${orderCreatedUiId.slice(-8).toUpperCase()}؟` });
  await expect(dialog).toBeVisible();
  await dialog.getByRole("button", { name: "إلغاء الطلب" }).click();

  await expect(page.getByText("تم إلغاء الطلب").first()).toBeVisible();
  await expect(card.getByText("ملغي", { exact: true })).toBeVisible();

  const order = await prisma.order.findUniqueOrThrow({ where: { id: orderCreatedUiId } });
  expect(order.status).toBe("CANCELLED");
  const inv = await prisma.partnerInventory.findUniqueOrThrow({
    where: { partnerId_variantId: { partnerId, variantId } },
  });
  // qty 2 released (started at 5 reserved for the two CREATED orders).
  expect(inv.stockReserved).toBe(3);
});

test("two parallel cancel requests on one CREATED order: exactly one 200 and one 409", async ({ page }) => {
  await loginViaUi(page);

  const [a, b] = await Promise.all([
    page.request.patch(`/api/profile/orders/${orderCreatedRaceId}/cancel`),
    page.request.patch(`/api/profile/orders/${orderCreatedRaceId}/cancel`),
  ]);
  const statuses = [a.status(), b.status()].sort();
  expect(statuses).toEqual([200, 409]);

  const order = await prisma.order.findUniqueOrThrow({ where: { id: orderCreatedRaceId } });
  expect(order.status).toBe("CANCELLED");
  const auditRows = await prisma.orderAuditLog.findMany({
    where: { orderId: orderCreatedRaceId, event: "cancelled" },
  });
  expect(auditRows).toHaveLength(1);
  expect(auditRows[0].details).toMatchObject({ cancelledBy: "customer" });

  const inv = await prisma.partnerInventory.findUniqueOrThrow({
    where: { partnerId_variantId: { partnerId, variantId } },
  });
  expect(inv.stockReserved).toBe(0);
});

test("a CONFIRMED order offers no cancel button and its endpoint returns 409", async ({ page }) => {
  await loginViaUi(page);
  await page.goto("/profile/orders");

  const card = page.locator("article", { hasText: `#${orderConfirmedId.slice(-8).toUpperCase()}` });
  await expect(card.getByRole("button", { name: "إلغاء الطلب" })).toHaveCount(0);

  const res = await page.request.patch(`/api/profile/orders/${orderConfirmedId}/cancel`);
