...
test.beforeAll(async () => {
  ...
  const variant = await prisma.variant.create({
    data: { productId, sku: `TEST-ADM-SHL-${uniqueSuffix}`, name: "M", pricePiastres: 5000, stockAvailable: 0, stockReserved: 0 },
  });
...
test.afterAll(async () => {
  if (couponId) {
    await prisma.adminAuditLog.deleteMany({ where: { entityId: couponId } });
