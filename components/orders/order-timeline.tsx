...
export function OrderTimeline({ auditLog, sla, awaitingConfirmLabel }: OrderTimelineProps) {
  return (
    <>
      <PanelCard title="سجل الطلب" icon={<Clock className="h-5 w-5 text-lapis-800" />}>
        <ol className="space-y-0">
          {auditLog.length === 0 && <p className="text-sm text-ink-soft">لا توجد أحداث مسجلة بعد.</p>}
          {auditLog.map((entry, idx) => (
            <li key={entry.id} className="flex gap-3 pb-4 last:pb-0">
              <div className="flex flex-col items-center">
                ...
                {idx < auditLog.length - 1 && <span className="mt-1 w-0.5 flex-1 bg-stone-100" />}
              </div>
              <div>
                <p className="text-[13px] font-bold text-ink">{describeOrderAuditEntry(entry)}</p>
