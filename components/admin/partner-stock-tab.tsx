...
import { Boxes, Loader2, Package, RefreshCw, Save } from "lucide-react";
...
/**
 * المخزون tab on the partner profile (backlog 9.4a (e)+(g)) — the editable grid moved
 * verbatim from `/admin/partner-inventory` (search, low-only, needs-setup, per-row
 * correction with reason → the existing `POST /api/admin/partner-inventory`, which already
 * writes the `MANUAL_ADJUSTMENT` ledger row), now scoped to one partner via a `partnerId`
 * prop instead of a page-level picker.
 */
...
export function PartnerStockTab({ partnerId }: { partnerId: string }) {
  ...
  React.useEffect(() => setPage(0), [debouncedQ, lowOnly, needsSetupOnly]);

  const load = React.useCallback(async () => {
    setFetching(true);
    const params = new URLSearchParams({ partnerId, limit: String(pageSize), offset: String(page * pageSize) });
  ...
  const rows = React.useMemo(() => products.flatMap((product) => product.variants.map((variant) => ({ product, variant }))), [products]);
  ...
  async function saveStock(variant: VariantRow) {
    ...
    if (stockAvailable < variant.stockReserved) {
      toast({ title: `لا يمكن أن يكون المخزون أقل من المحجوز (${variant.stockReserved})`, variant: "destructive" });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <PanelCard
        title="مخزون هذا الشريك"
        icon={<Package className="h-5 w-5 text-lapis-800" />}
        toolbar={
          <div className="flex flex-wrap items-center gap-2">
            <Button type="button" size="sm" variant={lowOnly ? "default" : "outline"} className="rounded-md" onClick={() => setLowOnly((v) => !v)}>
              مخزون منخفض فقط
            </Button>
            <Button type="button" size="sm" variant={needsSetupOnly ? "default" : "outline"} className="rounded-md" onClick={() => setNeedsSetupOnly((v) => !v)}>
              يحتاج إعداد فقط
            </Button>
            <SearchInput value={search} onChange={setSearch} placeholder="بحث بالمنتج أو SKU…" />
            <Button type="button" size="sm" variant="outline" className="rounded-md" onClick={load} disabled={fetching}>
      ...
      >
        {loading ? (
          ...
          <EmptyState icon={<Package className="h-12 w-12" />} title={debouncedQ || lowOnly || needsSetupOnly ? "لا توجد نتائج مطابقة" : "لا توجد منتجات"} />
        ) : (
          <TableScroll>
                    ...
                    <TableRow key={variant.id}>
                      ...
                      <TableCell>
                        <div className="flex min-w-32 items-center gap-2">
                          {variant.colorHex && <span className="h-4 w-4 shrink-0 rounded-full border border-border" style={{ backgroundColor: variant.colorHex }} />}
                          <span>{variantLabel(variant)}</span>
                          {variant.inventoryId === null && <Badge variant="outline" className="text-[11px]">يحتاج إعداد</Badge>}
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge variant={variant.sellable > 0 ? "default" : "destructive"}>{formatNumberEn(variant.sellable)}</Badge>
                      </TableCell>
                      <TableCell className="whitespace-nowrap text-xs text-muted-foreground">{variant.updatedAt ? formatDateEn(variant.updatedAt) : "لم يسجل"}</TableCell>
                      <TableCell className="text-left">
                        <Button type="button" size="sm" variant={dirty ? "default" : "outline"} className="rounded-md" disabled={!dirty || savingVariantId === variant.id} onClick={() => saveStock(variant)}>
                          {savingVariantId === variant.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
                        </Button>
                      </TableCell>
                    </TableRow>
                  );
        ...
        {total > 0 && <PaginationBar className="mt-6" page={page} pageSize={pageSize} total={total} onPageChange={setPage} onPageSizeChange={setPageSize} disabled={fetching} />}
      </PanelCard>
    </div>
  );
}
