 ...
 * `.../requests`). Each tab is its own route/page — this component only supplies the shared
 * chrome around them.
 *
 ...
 *
 * Split out of `app/(partner)/partner/stock/layout.tsx` (backlog 10.30): that file is now a
 * server component so it can export `metadata` for the `/partner/stock` route itself (a
 * client-component page cannot); this client piece keeps the `usePathname` chrome logic
 * unchanged.
 */
